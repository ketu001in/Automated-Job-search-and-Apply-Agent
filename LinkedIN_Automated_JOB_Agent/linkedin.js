...
const store       = require('../main/store');   // global app-settings + questionBank (shared)
...
async function runLinkedInAgent(log) {
  ...
  // Load question bank so self-learned answers are available this run (shared globally)
  formFiller.loadQuestionBank(store.get('questionBank', []));
    ...
    for (const jobId of jobIds) {
      ...
      try {
        ...
        if (result === true) {
          ...
          if (sessionQA.length) {
            const bank = store.get('questionBank', []);
            ...
            store.set('questionBank', bank.slice(-500)); // keep last 500 unique questions
...
function waitForResume() {
  return new Promise(resolve => {
    const check = setInterval(() => {
      ...
      // or the emergency Stop button was pressed) — otherwise a paused run with
      // a closed browser would wait forever with no way to recover.
      if (!agentState.isPaused() || agentState.isStopped()) {
        clearInterval(check);
        resolve();
      }
    }, 500);
      ...
      try {
        ...
        for (let i = 0; i < labels.length; i++) {
          if (labels[i].toLowerCase() === (ans||'').toLowerCase() ||
            ...
            log(`   Radio "${legend}": ${labels[i]}`, 'info');
            matched = true;
        ...
        if (!matched) {
          ...
          if (yesIdx >= 0) { await radios[yesIdx].click(); log(`   Radio "${legend}" → Yes (default)`, 'info'); }
          else if (radios.length) { await radios[0].click(); log(`   Radio "${legend}" → ${labels[0]} (default)`, 'info'); }
          await page.waitForTimeout(300);
        }
      } catch { /* skip */ }
    }
  ...
  for (const fieldset of await page.$$('.jobs-easy-apply-modal fieldset')) {
    try {
      ...
      for (let i = 0; i < cbLabels.length; i++) {
        ...
        if (matches && !isChecked) { await checkboxes[i].click(); log(`   Checked "${legend}" → "${cbLabels[i]}"`, 'info'); }
      }
    } catch { /* skip */ }
  }
}
